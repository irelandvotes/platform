...
const nationalResults = (() => {
  ...
  Object.entries(results || {}).forEach(([constituency, constituencyData]) => {
    const counts = constituencyData?.counts;
    if (!counts) return;
...
Quota: <b>{current.data.counts[count][0].quota?.toLocaleString()}</b>
</div>

<div style={{ marginBottom: "6px" }}>
Electorate: <b>{current.data.counts[count][0].electorate?.toLocaleString()}</b>
</div>

<div style={{ marginBottom: "6px" }}>
Turnout: <b>{current.data.counts[count][0].turnout?.toLocaleString()}</b>
</div>

<div style={{ marginBottom: "6px" }}>
TVP: <b>{current.data.counts[count][0].tvp?.toLocaleString()}</b>
</div>

<div>
Spoilt: <b>{current.data.counts[count][0].spoilt?.toLocaleString()}</b>
</div>
